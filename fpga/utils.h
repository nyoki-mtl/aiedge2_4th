...
void correct_region_boxes(vector<vector<float>>& boxes, int n,
    ...
    for (int i = 0; i < n; ++i){
        ...
        boxes[i][2] *= (float)netw/new_w;  // 比率に合わせてboxの幅修正
        boxes[i][3] *= (float)neth/new_h;  // 比率に合わせてboxの高さ修正
    }
}
...
void detect(vector<vector<float>> &boxes, vector<float> result,
	...
	vector<float> biases{81,82, 135,169, 344,319, 10,14, 23,27, 37,58};  // バイアスをtiny-yoloに合わせる
    ...
    for (int h = 0; h < height; ++h) {  // 0-416まで繰り返し
        for (int w = 0; w < width; ++w) {  // 0-416まで繰り返し
            for (int c = 0; c < anchorCnt; ++c) {
                ...
                box.push_back((w + sigmoid(swap[h * width + w][c][0])) / width);  // 1要素目はx
                box.push_back((h + sigmoid(swap[h * width + w][c][1])) / height);  // 2要素目はy
                // ここでcはカウント、anchorCntは出力層ごとのアンカー総数、numは出力層数
                box.push_back(exp(swap[h * width + w][c][2]) * biases[2 * c + 2 * anchorCnt * num] / float(sWidth));  // 3要素目は幅
                box.push_back(exp(swap[h * width + w][c][3]) * biases[2 * c + 2 * anchorCnt * num + 1] / float(sHeight));  // 4要素目は高さ
                box.push_back(-1);  // 5要素目は暫定的に-1
                box.push_back(obj_score);  // 6要素目は確信度
                for (int p = 0; p < classificationCnt; p++) {
                    box.push_back(obj_score * sigmoid(swap[h * width + w][c][5 + p]));  // 7-12要素目は各クラスに応じた確信度
                }
                boxes.push_back(box);
            }
        }
...
{
    image resized = make_image(w, h, im.c);  // 変換後のw * 変換後のh * 元画像のc の空のイメージを作成
    image part = make_image(w, im.h, im.c);  // 変換後のw * 元画像のh * 元画像のc の空のイメージを作成
    int r, c, k;
    float w_scale = (float)(im.w - 1) / (w - 1);  // wの拡大率の逆数を計算
    float h_scale = (float)(im.h - 1) / (h - 1);  // hの拡大率の逆数を計算
    for(k = 0; k < im.c; ++k){
        for(r = 0; r < im.h; ++r){
            for(c = 0; c < w; ++c){
                float val = 0;
...
image resize_image_nn(image im, int w, int h)
{
    image resized = make_image(w, h, im.c);  // 変換後のw * 変換後のh * 元画像のc の空のイメージを作成
    int r, c, k;
    float w_scale = (float)im.w / (float)w;  // wの拡大率の逆数を計算
    float h_scale = (float)im.h / (float)h;  // hの拡大率の逆数を計算
    float px, py;
    float val;

    for(k = 0; k < im.c; ++k){
        for(r = 0; r < h; ++r){  // 変換後のhまで繰り返し
            for(c = 0; c < w; ++c){  // 変換後のwまで繰り返し
            	px = (float)c * w_scale;
            	py = (float)r * h_scale;
            	val = get_pixel(im, px, py, k);
            	set_pixel(resized, c, r, k, val);
            }
        }
    }
...
image load_image(cv::Mat& img, const string& filename) {
